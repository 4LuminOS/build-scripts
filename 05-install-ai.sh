...
# Version: 0.1.8 (Reverted to download in chroot)
# ==============================================================================
set -e
...
# --- Configure the Service and Lumin Model inside Chroot ---
cat > "$LUMINOS_CHROOT_DIR/tmp/configure_ai.sh" << EOF
#!/bin/bash
set -e
export DEBIAN_FRONTEND=noninteractive
echo "--> Creating dedicated 'ollama' user inside chroot..."
# User home is /usr/share/ollama where models downloaded by pull command should be
useradd -r -s /bin/false -m -d /usr/share/ollama ollama
echo "--> Setting ownership of model files for ollama user..."
# Ensure the directory exists from the pull command
...
# Create the model using the binary and the downloaded files
