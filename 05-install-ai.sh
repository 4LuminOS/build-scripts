...
# Standard location where Ollama stores models when run by user
# Check ~/.ollama/models if this one doesn't exist after user pull
...
if [ ! -d "${HOST_MODEL_DIR}" ]; then
    ...
    # Attempt alternative common path
    if [ -d "$HOME/.ollama/models" ]; then
         echo "INFO: Found models at $HOME/.ollama/models instead. Please update HOST_MODEL_DIR in script 05."
    fi
    exit 1
fi
echo "--> Found Ollama models on host system."
